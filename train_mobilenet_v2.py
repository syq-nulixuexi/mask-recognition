...
train_loss_results = []  # 将每轮的train loss记录在此列表中，为后续画loss曲线提供数据
train_acc = []  # 将每轮的train acc记录在此列表中，为后续画acc曲线提供数据
val_loss_results = []  # 将每轮的val loss记录在此列表中，为后续画loss曲线提供数据
val_acc = []  # 将每轮的val acc记录在此列表中，为后续画acc曲线提供数据


def main():
    data_root = os.getcwd()  # 获取此时所在目录位置
    image_path = os.path.join(data_root, "image")  # 存放图片位置
    train_dir = os.path.join(image_path, "train")  # train文件夹
    validation_dir = os.path.join(image_path, "val")  # val文件夹
